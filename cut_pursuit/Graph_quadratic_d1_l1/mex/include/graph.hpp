...
========================
    ...
    Modified by Hugo Raguet (hugo.raguet@gmail.com)
    for use with Cut-Pursuit approach (2016)

     - reuse tree option removed, together with dedicated variables
     - class attributes node and arc made public for direct manipulations
     - arc attribute is_active tells if an edge is activated
     - boolean values (is_sink, is_active) stored over 1 byte only (uint8_t)
*/
...
public:
	...
	/*  must be uncommented for reuse_tree
...
public:
	...
	struct node
	{
		...
		uint8_t		is_sink : TRUE;	// flag showing whether the node is in the source or in the sink tree (if parent!=NULL)
        /*
	...
	struct arc
	{
        ...
        uint8_t     is_active;  // tells if in the cut pursuit active set
	};
...
{
    if (error_function) (*error_function)("reuse tree not implemented for saving space");
    exit(1);
}
/*
	node* i = nodes + _i;
