/*==================================================================
 * [Cv, rX, CP_it, Time, Obj, Dif] = CP_PFDR_graph_quadratic_d1_bounds_mex(Y, A, Eu, Ev, La_d1, Bnd, CP_difTol, CP_itMax, PFDR_rho, PFDR_condMin, PFDR_difRcd, PFDR_difTol, PFDR_itMax, verbose)
 * 
 *  Hugo Raguet 2016
 *================================================================*/

#include "mex.h"
#include "../include/CP_PFDR_graph_quadratic_d1_bounds.hpp"

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    const int N = mxGetM(prhs[1]);
    const int V = mxGetN(prhs[1]);
    const int E = mxGetNumberOfElements(prhs[2]);
    const int *Eu = (int*) mxGetData(prhs[2]);
    const int *Ev = (int*) mxGetData(prhs[3]);
    const int CP_itMax = (int) mxGetScalar(prhs[7]);
    const int PFDR_itMax = (int) mxGetScalar(prhs[12]);
    ...
    if (mxIsDouble(prhs[0])){
        ...
        const double *A = (double*) mxGetData(prhs[1]);
        const double *La_d1 = (double*) mxGetData(prhs[4]);
        const double *Bnd = (double*) mxGetData(prhs[5]);
        const double CP_difTol = (double) mxGetScalar(prhs[6]);
        const double PFDR_rho = (double) mxGetScalar(prhs[8]);
        const double PFDR_condMin = (double) mxGetScalar(prhs[9]);
        const double PFDR_difRcd = (double) mxGetScalar(prhs[10]);
        const double PFDR_difTol = (double) mxGetScalar(prhs[11]);
        ...
        CP_PFDR_graph_quadratic_d1_bounds<double>(V, E, N, &rV, Cv, &rX, Y, A, \
                                Eu, Ev, La_d1, Bnd[0], Bnd[1], CP_difTol, \
        ...
        const float *Bnd = (float*) mxGetData(prhs[5]);
        const float CP_difTol = (float) mxGetScalar(prhs[6]);
        const float PFDR_rho = (float) mxGetScalar(prhs[8]);
        const float PFDR_condMin = (float) mxGetScalar(prhs[9]);
        const float PFDR_difRcd = (float) mxGetScalar(prhs[10]);
        const float PFDR_difTol = (float) mxGetScalar(prhs[11]);
        ...
        CP_PFDR_graph_quadratic_d1_bounds<float>(V, E, N, &rV, Cv, &rX, Y, A, \
                                Eu, Ev, La_d1, Bnd[0], Bnd[1], CP_difTol, \
                                CP_itMax, CP_it, PFDR_rho, PFDR_condMin, \
                                PFDR_difRcd, PFDR_difTol, PFDR_itMax, \
                                Time, Obj, Dif, verbose, NULL);
    ...
    mxSetM(plhs[1], rV);
    /* check inputs
    mexPrintf("V = %d, E = %d, N = %d, Y[0] = %g, A[0] = %g\n \
    Eu[0] = %d, Ev[0] = %d, La_d1[0] = %g, Bnd[0] = %g,, Cv[0] = %d\n \
    CP_difTol = %g, CP_itMax = %d, *CP_it = %d,\n \
    timeRec = %d, objRec = %d, difRec = %d verbose = %d\n", \
    V, E, N, Y[0], A[0], Eu[0], Ev[0], La_d1[0], Bnd[0], Cv[0], \
