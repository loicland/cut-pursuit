...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    const int itMax = (int) mxGetScalar(prhs[10]);
    const int verbose = (int) mxGetScalar(prhs[11]);
    ...
    if (mxIsDouble(prhs[0])){
        ...
        const double rho = (double) mxGetScalar(prhs[6]);
        const double condMin = (double) mxGetScalar(prhs[7]);
        const double difRcd = (double) mxGetScalar(prhs[8]);
        const double difTol = (double) mxGetScalar(prhs[9]);
        ...
        PFDR_graph_quadratic_d1_bounds<double>(V, E, 0, X, Y, La_l2, Eu, Ev, La_d1, \
                                    Bnd[0], Bnd[1], DIAG, La_l2, rho, condMin, \
        ...
        const float rho = (float) mxGetScalar(prhs[6]);
        const float condMin = (float) mxGetScalar(prhs[7]);
        const float difRcd = (float) mxGetScalar(prhs[8]);
        const float difTol = (float) mxGetScalar(prhs[9]);
        ...
        PFDR_graph_quadratic_d1_bounds<float>(V, E, 0, X, Y, La_l2, Eu, Ev, La_d1, \
                                    Bnd[0], Bnd[1], DIAG, La_l2, rho, condMin, \
                                    difRcd, difTol, itMax, it, Obj, Dif, verbose);
                                   /* 22 arguments */
    ...
    /* check inputs
    mexPrintf("V = %d, E = %d, N = %d, Y[0] = %g, A[0] = %g, X[0] = %f\n \
    Eu[0] = %d, Ev[0] = %d, La_d1[0] = %g, Bnd[0] = %g, Ltype = %d, L[0] = %g\n \
    rho = %g, condMin = %g, difRcd = %g, difTol = %g, itMax = %d, *it = %d,\n \
    objRec = %d, difRec = %d verbose = %d\n", \
    V, E, N, Y[0], A[0], X[0], Eu[0], Ev[0], La_d1[0], Bnd[0], Ltype, L[0], rho, \
