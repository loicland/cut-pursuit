...
namespace CP {
template <typename T>
class CutPursuit_KL : public CutPursuitProblem<T>
{
    public:
    virtual std::pair<T,T> compute_energy() override
    {
        ...
        VertexIterator<T> i_ver, i_ver_end;
             ...
             i_ver!= this->lastIterator; ++i_ver)
        {
            for(int i_dim=0; i_dim<this->dim; i_dim++)
            {
                ...
                //std::cout << smoothedObservation << " " << smoothedValue << std::endl;
            }
            //std::cout << "=====================" << std::endl;
        }
...
//      //-----because we use the KLdivergence we need to use a custom kmeans implementation
//        VertexAttributeMap<T> vertex_attribute_map
//                = boost::get(boost::vertex_bundle, this->mainGraph);
//        VertexIndexMap<T> indexMap = boost::get(boost::vertex_index, this->mainGraph);
//        int i_com_int = -1;
//        cv::Mat centroids(2, this->dim, CV_32F,0.), labels_kmean(i_com->size(), 1, CV_32F,0.);
//        //==first we randomly initialize the centroids ===
//        std::size_t indexCentroid1, indexCentroid2;

//        //==TODO: initialize with PCA...
//        for (std::size_t ite_km = 0; ite_km < this->parameter.kmeans_ite; ite_km++)
//        {
...
//                continue;
//            }
//            cv::Mat data(i_com->size(), this->dim, CV_32F,0.), labels_kmean(i_com->size(), 1, CV_32F,0.);
...
//                    if (vertex_attribute_map[*i_ver].weight>0)
//                    {
//                        labels[indexMap(*i_ver)] = (labels_kmean.at<float>(indexInData)==0.f);
//                        indexInData++;
//                    }
//                    else
//                    {
            ...
            for (int i_ver = 0;  i_ver < this->components[i_com].size(); i_ver++)
            {
                ...
                for(int i_dim=0; i_dim < this->dim; i_dim++)
                {
                    ...
                    smoothedValueB =
                             this->parameter.smoothing / this->dim
                           + (1 - this->parameter.smoothing)
                           *  centers[i_com][0][i_dim];
                    smoothedValueNotB =
                             this->parameter.smoothing / this->dim
                           + (1 - this->parameter.smoothing)
                           *  centers[i_com][1][i_dim];
