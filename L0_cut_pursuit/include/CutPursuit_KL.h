...
class CutPursuit_KL : public CutPursuit<T>
{
      ...
      //--------loading structures---------------------------------------------------------------
        ...
        for (std::size_t i_step = 1; i_step <= this->parameter.flow_steps; i_step++)
        {
            ...
            boost::boykov_kolmogorov_max_flow(
                       ...
                       this->source,
                       this->sink);
            for (std::size_t i_com = 0; i_com < nb_comp; i_com++)
            {
                if (this->saturated_components[i_com])
                {
                    continue;
                }
                for (std::size_t i_ver = 0;  i_ver < this->components[i_com].size(); i_ver++)
                {
    ...
    { //-----initialize the labelling for each components with kmeans------------------------------
        ...
        for (std::size_t i_com = 0; i_com < nb_comp; i_com++)
        {
            std::size_t comp_size = this->components[i_com].size();
            ...
            for (std::size_t init_kmeans = 0; init_kmeans < this->parameter.kmeans_resampling; init_kmeans++)
            {
                ...
                std::size_t first_kernel  = std::rand() % comp_size, second_kernel = 0;
                for(std::size_t i_dim=0; i_dim < this->dim; i_dim++)
                ...
                //now compute the square distance of each point to this kernel
    ...
    //=================================   COMPUTE_VALUE   =========================================
    //=============================================================================================
    virtual std::pair<std::vector<T>, T> compute_value(const std::size_t & i_com) override
    {
        VertexAttributeMap<T> vertex_attribute_map
                                    = boost::get(boost::vertex_bundle, this->main_graph);
        T total_weight = 0;
        ...
        for (std::size_t ind_ver = 0; ind_ver < this->components[i_com].size(); ++ind_ver)
        {
