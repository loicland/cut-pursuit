...
struct CPparameter
{
    ...
    std::size_t flow_steps; //number of steps in the optimal binary cut computation
    std::size_t kmeans_ite; //number of iteration in the kmeans sampling
    std::size_t kmeans_resampling; //number of kmeans re-intilialization
    std::size_t verbose; //verbosity
    std::size_t max_ite_main; //max number of iterations in the main loop
...
class CutPursuit
{
    ...
    std::size_t dim;     // dimension of the data
    std::size_t nVertex; // number of data point
    std::size_t nEdge;   // number of edges between vertices (not counting the edge to source/sink)
    ...
    std::pair<std::vector<T>, std::vector<T>> run()
    {
        ...
        TimeStack ts; ts.tic();
        //the main loop
        for (std::size_t ite_main = 1; ite_main <= this->parameter.max_ite_main; ite_main++)
        {
            //--------those two lines are the whole iteration-------------------------
            std::size_t saturation = this->split(); //compute optimal binary partition
    ...
    //=================================   COMPUTE_VALUE   =========================================
    //=============================================================================================
    virtual std::pair<std::vector<T>, T> compute_value(const std::size_t & ind_com)
    {
...
//=============================================================================================
    ...
    void initialize()
    {
        ...
        std::size_t eIndex = boost::num_edges(this->main_graph);
    ...
    {
        for (std::size_t ind_com = 0;  ind_com < this->components.size(); ++ind_com)
    ...
    {  //this function compute the connected components of the graph with active edges removed
        ...
        //#pragma omp parallel for if (this->parameter.parallel) schedule(dynamic)
        for (std::size_t ind_com = 0; ind_com < this->root_vertex.size(); ind_com++)
        {
             ...
             ite_ver != this->lastIterator; ++ite_ver)
        {
            ...
            std::size_t current_component_size =
    ...
    inline std::vector<VertexDescriptor<T>> connected_comp_from_root(const VertexDescriptor<T> & root
                , const std::size_t & size_comp, std::vector<bool> & vertices_seen , std::vector<bool> & edges_seen)
    {
    ...
    {   //compute the adjacency structure between components as well as weight and value of each component
        ...
        //#pragma omp parallel for schedule(dynamic) if (this->parameter.parallel)
        for (std::size_t ind_com = 0;  ind_com < this->components.size(); ind_com++)
        {
        ...
        std::size_t ind_border_edge = 0, comp1, comp2, component_source, component_target;
    ...
    void merge()
    {
        ...
        VertexDescriptor<T> source_component, target_component;
        std::size_t ind_source_component, ind_target_component, border_edge_currentIndex;
        ...
        std::size_t ind_new_component = 0;
        for (std::size_t ind_com = 0; ind_com < this->components.size(); ind_com++)
        {
...
//==========================saturateComponent====================================================
//===============================================================================================
    inline void saturateComponent(const std::size_t & ind_com)
